...
if [ -v USER_ID ] && [ "$USER_ID" != "$DEFAULT_USER_ID" ]; then
    echo "Changing robot user ID to match your host's user ID ($USER_ID)."
    echo "This operation can take a while..."

    usermod --uid $USER_ID robot

    # Ensure all files in the home directory are owned by the new user ID
    find /home/robot -user $DEFAULT_USER_ID -exec chown -h $USER_ID {} \;
fi
...
# Run the provided command using user 'robot'
exec "$@"